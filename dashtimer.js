...
function DashTimer(div) {
     ...
     */
    self.start = function(duration,start,finish, simbol) {
        resetPromise_();
        dash_.internal.duration = fixDef_ (duration,dash_.duration);
        return work_(fixDef_ (start,0) , dash_.internal.duration, fixDef_ (finish,1), simbol);
    };
     ...
     * @return {Promise} a promise that'll be resolved when it times out
     */
    function work_(start, duration, finish, simbol) {
        ...
        show_(start, duration, finish, simbol)
            .then(
                function(expired) {
                    self.resolve();
                },
     ...
     * @return {Promise} a promise that'll be resolved when it times out
     */
    function show_(startAt, duration, finishAt, simbol) {
            ...
            dInfo_.path.transition(dash_.name)
                ...
                .attrTween("d", function(d) {
                    ...
                    return function(t) {
                        ...
                        if (d.values.show) {
                            dInfo_.textElements.text(d.values.decorate(d.value).toString()+simbol);
                        }
                        return dInfo_.arc(d);
                    };
                })
